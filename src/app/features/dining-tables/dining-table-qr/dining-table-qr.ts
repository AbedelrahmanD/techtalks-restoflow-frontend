...
// TODO: replace with the real public-facing domain once confirmed with the team.
const MENU_DOMAIN = 'Domain.com';
  ...
  ngOnInit(): void {
    ...
    this.diningTablesService.getTable(id).subscribe({
      next: (table) => {
        ...
        this.qrValue.set(`${MENU_DOMAIN}/menu/${table.qrCodeToken}`);
        this.loading.set(false);
